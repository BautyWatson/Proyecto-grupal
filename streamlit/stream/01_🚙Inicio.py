...
with col1:
    tit1= '<p style="font-family:sans-serif; color:#124183; text-align:center; font-size: 24px;"><b>Clientes</b></p>'
    st.markdown(tit1, unsafe_allow_html=True)
    st.image(image1)
    agree1 = st.checkbox('Cotiza tu poliza')
    if agree1:
        var=1
   # load = st.button('Cotiza tu poliza')
...
with col2:
    tit2= '<p style="font-family:sans-serif; color:#124183; text-align:center; font-size: 24px;"><b>Agente de seguros</b></p>'
    st.markdown(tit2, unsafe_allow_html=True)
    st.image(image2)
    agree = st.checkbox('Evalua el riesgo de un usuario')
    if agree:
        var = 2
    #load1 =st.button('Evalua el riesgo de un usuario')
...
#opcion = st.checkbox('Cotiza tu poliza')
#opcion1 = st.checkbox('Aseguradoras')
#st.markdown('## **¿Quieres conocer el costo de tu poliza o eres un agente de seguros?**')
#opciones = [" ", 'Cotiza tu poliza', 'Agente de seguros']
#app = st.selectbox('Elige el servicio al que deseas acceder', opciones,0)
#if app =='Cotiza tu poliza':
